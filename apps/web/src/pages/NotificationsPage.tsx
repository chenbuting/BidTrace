...
export function NotificationsPage() {
  const { user, refreshNotifyCount } = useOutletContext<{
    user: UserInfo | null;
  ...
  useEffect(() => {
    if (!canView) return;
    void loadInbox(1, pageSize, unreadOnly);
  ...
  const onOpenItem = async (item: NotifyItem) => {
    ...
    if (item.is_unread || !item.read_at) {
      try {
        ...
        setItems((prev) =>
          prev.map((x) =>
            x.id === item.id ? { ...x, is_unread: 0, read_at: x.read_at || "已读" } : x,
          ),
        );
        refreshNotifyCount?.();
      } catch {
    ...
    <div className="p-6 md:p-8">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[#6b6b6b]">接收提醒；有权限的人可选择接收人发送</p>
        </div>
          ...
          <div className="overflow-hidden rounded-xl border border-black/[0.08] bg-white">
            {items.length === 0 ? (
              ...
              <ul className="divide-y divide-black/[0.05]">
                {items.map((item) => {
                  const unread = Boolean(item.is_unread) || !item.read_at;
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    size="sm"
                    type="button"
                    onClick={() => setSelected(new Set(picker.map((u) => u.id)))}
                  >
                    全选
                  </Button>
