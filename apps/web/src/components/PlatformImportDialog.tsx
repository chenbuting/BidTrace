...
/** 平台账号导入：选模式 → 增量冲突人工选择 / 全部覆盖确认 */
export function PlatformImportDialog({ file, onClose, onDone }: Props) {
  ...
  useEffect(() => {
    void fetchLatestPlatformBackup()
  ...
  const runPreview = async (nextMode: Mode) => {
    ...
    setError("");
    try {
      const data = await previewPlatformImport(file, nextMode);
      setTotal(data.total);
  ...
  const doCommit = async (
    ...
    try {
      ...
      const r = await commitPlatformImport(file, nextMode, list);
      const msg =
        nextMode === "full"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-white shadow-lg">
        <div className="border-b border-black/[0.06] px-5 py-4">
          <h3 className="text-[15px] font-semibold text-[#26251e]">导入平台账号</h3>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[12px] leading-relaxed text-[#6b6b6b]">
                      新平台直接写入。若「平台名称 + 平台网址」都与库中已有记录相同，会单独列出不一样的字段，由你选择
                      <b>保留原数据</b>或<b>用 Excel 覆盖</b>。表头必须与固定模板完全一致。
                    </p>
                  </div>
                </div>
              </label>
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[12px] leading-relaxed text-[#6b6b6b]">
                      先自动备份当前全部平台账号，再清空表，再导入 Excel 全部内容。之后可用「恢复上一版」找回覆盖前数据。表头必须与固定模板完全一致，否则会拒绝导入。
                    </p>
                  </div>
                </div>
              </label>
          ...
          {step === "conflicts" ? (
            <div className="space-y-3">
              <div className="rounded-lg bg-[#fff7f0] px-3 py-2 text-[12px] text-[#b54708]">
                发现 {conflicts.length} 条「名称+网址」相同记录；新增约 {newCount} 条。请逐条选择保留或覆盖。
              </div>
              {conflicts.map((c) => (
                <div key={c.row_index} className="rounded-xl border border-black/[0.08] p-3">
                  <p className="text-[13px] font-medium text-[#26251e]">{c.name}</p>
                  <p className="truncate text-[11px] text-[#6b6b6b]">{c.url}</p>
                  {c.identical ? (
